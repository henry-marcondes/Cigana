...
async function executarTestes() {
    ...
    console.log('\n===================================');
    console.log('TESTES MODEL: Favorito');
    console.log('===================================\n');
    ...
    try {
        ...
        const usuario = await Usuario.criar({
            email: `favorito${timestamp}@ciganas.com`,
        ...
        const favorito = await Favorito.criar({
        ...
        console.log(
            await Favorito.listarPorUsuario(usuario.id)
        );

        console.log('\n5. Buscar por ID');

        console.log(
            await Favorito.buscarPorId(favorito.id)
        );

        console.log('\n6. Buscar por usuário e livro');

        console.log(
            await Favorito.buscarPorUsuarioELivro(
                usuario.id,
                livro.id
            )
        );
        ...
        console.log(
            await Favorito.desativar(
                favorito.id
            )
        );

        console.log('\n8. Buscar favorito desativado');

        console.log(
            await Favorito.buscarPorId(
                favorito.id
            )
        );
